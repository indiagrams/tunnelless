...
- **App Review was told the demo account was not required, on a record that still held one.** `deliver`'s `review_information()` strips every review field and omits the empty ones from its `appStoreReviewDetail` PATCH — correct, and what `fastlane/Fastfile`'s `" "` sentinel relies on. But it finishes with an `else` branch that **writes** `demo_account_required = false` rather than omitting it, deriving the flag from the two values it just decided not to send.
  ...
  `APP_REVIEW_DEMO_USER` / `APP_REVIEW_DEMO_PASSWORD` are not set here, so the sentinel empties both, deliver omits them — leaving App Store Connect's stored credentials correctly untouched — and then flips the flag beside them. `0.1.0` was approved with `demoAccountRequired: true`; the first `make submit` for `0.1.1` produced `false` on **both** platforms with the inherited credentials still in place. A record that offers a demo account and marks it not required is one nobody chose.

  Caught by reading the review detail back from ASC before submitting, not from the exit code or deliver's log, and corrected by hand on both `0.1.1` records before they went to Apple.

  Fixed **upstream** in apple-shipkit ([#284](https://github.com/indiagrams/apple-shipkit/pull/284), merged as `e2d7cbf`) rather than here, since `bin/` is template-owned and `bin/submit.rb` was byte-identical to the template; cherry-picked in unmodified, the same route as [#283](https://github.com/indiagrams/apple-shipkit/pull/283) one commit earlier. `bin/submit.rb` now snapshots the review detail during its existing pre-flight and reconciles after the lanes, restoring the flag **only** when deliver changed it without changing the credentials it derives from — so a genuine credential change keeps deliver's honest derivation. Brings `bin/lib/review_detail.rb` (the decision as a pure function, no ASC account needed) and `test/demo_account_required_test.rb` — 14 assertions, mutation-verified: the unguarded behaviour kills 3, dropping the credentials-unchanged guard kills 3, removing the `to_s` coercion kills 2. Wired as the `demo-account-regression` job.

- **`make submit` staged a version that could not be submitted, and exited 0 doing it.** Two independent gaps in `bin/submit.rb`, both of which only appear on a fork's *first* marketing-version bump, and neither of which the exit code reveals.

  `deliver` writes metadata **into an existing editable App Store version** — it does not create one, and nothing else in the pipeline did either (`bootstrap_asc` verifies the *App* record, not a version). So the first `make submit` after bumping to `0.1.1` sat retrying `Cannot find edit app store version` until it gave up — and **returned 0**. Separately, neither `deliver` call passes `build_number`, so nothing attached a build. That is the worse of the two: a run that prints `All configured platforms staged` and exits 0, leaving both platforms on a version with `build=NONE` that App Store Connect will not accept a submission for. Visible only by querying ASC afterwards; invisible from the log, and invisible from `$?`.

  Fixed **upstream** in apple-shipkit ([#283](https://github.com/indiagrams/apple-shipkit/pull/283), merged as `212a948`) rather than here, because `bin/` is template-owned and `bin/submit.rb` was byte-identical to the template — verified by hashing this repo's copy against upstream's pre-fix blob (`a3a099d…`, identical), so the fix was authored there and pulled in **unmodified**. Same route as the `ship.rb` streaming fix ([#276](https://github.com/indiagrams/apple-shipkit/pull/276)) and the build-number fix ([#278](https://github.com/indiagrams/apple-shipkit/pull/278)).

  Adds a pre-flight ahead of every lane that creates the version record if missing, then attaches the newest `VALID` unexpired build for that marketing version and platform. Idempotent — an existing version is reused and an already-attached build is left alone, so re-running `make submit` is safe. A version in a state `deliver` cannot edit now **fails loudly** instead of retrying into a timeout, and calls out the common case by name: `READY_FOR_SALE` means the train is closed and the fix is a marketing bump, not a retry. Missing build likewise fails with `run make ship first` rather than staging an empty version.

  **Not exercised by a run in this repo.** The only way to exercise it is `make submit`, and `0.1.1` is currently staged awaiting a human Submit click — re-running the lanes would re-upload metadata against a version under review-preparation. Verified here by `ruby -c` and by byte-identity with the upstream blob that its own CI proved green; the behaviour was proven upstream.

- **Build numbers no longer restart at 1 after a marketing-version bump.** `Bootstrap::Version.next_build_number` scoped its App Store Connect query to the current marketing version, which is correct only while nothing has been released: Apple requires `CFBundleVersion` to exceed what was previously uploaded for the platform, and once a version is live that spans the app's whole history. Bumping 0.1.0 → 0.1.1 therefore produced build `1`, and `altool` refused it.

  It surfaced as **one `make ship` accepted by iOS and refused by macOS**: iOS's 0.1.0 had never been released so a new train may start at 1, while macOS's was live at build 7. Same command, two platforms, two answers — the difference was release status, not the build.

  Fixed **upstream** in apple-shipkit ([#278](https://github.com/indiagrams/apple-shipkit/pull/278)) rather than here, since `bin/` is template-owned and `bin/lib/version_resolver.rb` was byte-identical to the template; cherry-picked in, as with the `ship.rb` streaming fix. Now takes a global max across all builds, which changes nothing about which TestFlight bucket a build lands in — that is `CFBundleShortVersionString`. Brings `test/build_number_test.rb` (11 assertions, mutation-verified: the original bug kills 9, a lexicographic max kills 4) and `test/stubs/spaceship.rb`.
