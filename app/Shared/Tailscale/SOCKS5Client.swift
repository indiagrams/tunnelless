...
struct SOCKS5Client: Sendable {
    ...
    func get(host: String, port: UInt16, path: String = "/", timeout: TimeInterval = 20) async throws -> String {
        ...
        // 3. CONNECT. Send the destination as a domain name (0x03) unless it
        //    parses as IPv4 — MagicDNS names must resolve on tsnet's side, not ours.
        var req = Data([0x05, 0x01, 0x00])
        if let v4 = Self.ipv4Octets(host) {
            req.append(0x01); req.append(contentsOf: v4)
        } else {
            let h = Data(host.utf8)
            req.append(0x03); req.append(UInt8(h.count)); req.append(h)
        }
        req.append(UInt8(port >> 8)); req.append(UInt8(port & 0xFF))
        try await Self.send(conn, req)
        ...
        // Drain BND.ADDR + BND.PORT so the stream is positioned at the payload.
        switch reply[3] {
        case 0x01: _ = try await Self.receive(conn, exactly: 4 + 2)
        case 0x04: _ = try await Self.receive(conn, exactly: 16 + 2)
        case 0x03:
            let len = try await Self.receive(conn, exactly: 1)
            _ = try await Self.receive(conn, exactly: Int(len[0]) + 2)
        default: throw SOCKS5Error.badReply
        }

        // 4. Tunnelled HTTP/1.1. `Connection: close` makes EOF the end-of-body
        //    signal, so no chunked/Content-Length parsing is needed here.
